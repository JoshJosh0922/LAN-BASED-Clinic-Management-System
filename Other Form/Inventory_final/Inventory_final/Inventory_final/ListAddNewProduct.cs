            ...
            if (dt.Rows.Count > 0)
            {
                ...
                else if (count == 0)
                {
                    if (txtCode.Text != "" && txtName.Text != "")
                    {
                        ...
                        if (result > 0)
                        {
                            sql = "select DATE_FORMAT(date_received, '%Y-%m-%d') as received from batch";
                            cmd = new MySqlCommand(sql, conn);
                            cmd.ExecuteNonQuery();
                            da.SelectCommand = cmd;
                            da.Fill(dt);
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                var a = dt.Rows[0]["received"];
                                var b = expiry.Value.Date.ToString("yyyy-MM-dd");
                                sql = "INSERT INTO `productstocks`(`code`, `consumed`, `stocks`, `batch`, `date_received`, `expiry_date`) VALUES ('" + txtCode.Text + "','" + "0" + "','" + txtStock.Text + "','" + cboBatch.Text + "','" + a + "','" + b + "')";
                                cmd = new MySqlCommand(sql, conn);
                                result = cmd.ExecuteNonQuery();
                                if (result > 0)
                                {
