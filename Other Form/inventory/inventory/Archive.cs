...
namespace inventory
{
    public partial class Archive : Form
    {
        ...
        private void Archive_Load(object sender, EventArgs e)
        {
            ...
            sql = "select count(*) as counts from `archiveuniqueproductlist`";
        ...
        private void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                flpList.Controls.Clear();
                sql = "SELECT count(id) as count FROM `archiveuniqueproductlist` where code = '" + txtSearch.Text + "' || name = '" + txtSearch.Text + "' || category = '" + txtSearch.Text + "'";
                cmd = new MySqlCommand(sql, conn);
                cmd.ExecuteNonQuery();
                da.SelectCommand = cmd;
                da.Fill(dt);
        ...
        public void items()
        {
            ...
            sql = "SELECT COUNT(t2.code) as counts, t1.code, t1.name, t1.unit, t1.category, t2.batch, sum(t2.consumed) as totalconsumed, SUM(t2.stocks) as totalstocks, DATE_FORMAT(t2.expiry_date, '%b %e, %Y') as expiry FROM archiveuniqueproductlist as t1 LEFT join archiveproductstocks as t2 on t1.code = t2.code group by t1.code order by t1.id";
            ...
            if (dt1.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < datacount; i++)
                {
                    ...
                    batch.Location = new Point(550, 10);
                    ...
                    category.Location = new Point(701, 10);
                    ...
                    consumed.Location = new Point(861, 21);
                    ...
                    stock.Location = new Point(1007, 21);
                    ...
                    expiry.Location = new Point(1120, 21);
                    ...
                    btnEdit.Location = new Point(1294, 21);
                    btnEdit.BackgroundImage = Properties.Resources.restore2;
                    ...
                    delete.Location = new Point(1372, 21);
        ...
        public void searchedItem()
        {
            ...
            sql = "Select archiveuniqueproductlist.code, archiveuniqueproductlist.name, archiveuniqueproductlist.unit, archiveuniqueproductlist.category, archiveproductstocks.batch, sum(archiveproductstocks.consumed) as totalconsumed, SUM(archiveproductstocks.stocks) as totalstocks, DATE_FORMAT(archiveproductstocks.expiry_date, '%b %e, %Y') as expiry from archiveproductstocks INNER JOIN archiveuniqueproductlist on archiveproductstocks.code = archiveuniqueproductlist.code where archiveuniqueproductlist.code = '" + txtSearch.Text + "' || archiveuniqueproductlist.name = '" + txtSearch.Text + "' || archiveuniqueproductlist.category = '" + txtSearch.Text + "' group by archiveuniqueproductlist.code order by archiveuniqueproductlist.id";
            //sql = "select *, count(*) as count from uniqueproductlist";
            cmd = new MySqlCommand(sql, conn);
            cmd.ExecuteNonQuery();
            da.SelectCommand = cmd;
            ...
            if (dt1.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < datacount; i++)
                {
                    ...
                    batch.AutoSize = false;
                    batch.Size = new Size(191, 34);
                    batch.Location = new Point(447, 10);
                    batch.Font = new Font("Segoe UI Variable", 10);
                    batch.ForeColor = Color.Black;
                    batch.Text = Convert.ToString(dt1.Rows[i]["batch"]);
                    batch.TextAlign = ContentAlignment.MiddleLeft;
        ...
        void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            DialogResult dial = MessageBox.Show("Are you sure you want to restore this item?", "Restore", MessageBoxButtons.YesNo);
            if (dial == DialogResult.Yes)
            {
                sql = "INSERT INTO `uniqueproductlist` SELECT * from archiveuniqueproductlist where code = '" + g + "'";
                cmd = new MySqlCommand(sql, conn);
                result = cmd.ExecuteNonQuery();
                if (result > 0)
                {
                    sql = "DELETE FROM `archiveuniqueproductlist` WHERE  code = '" + g + "'";
                    cmd = new MySqlCommand(sql, conn);
                    result = cmd.ExecuteNonQuery();
                    if (result > 0)
                    {
                ...
                sql = "INSERT INTO `productstocks` SELECT * from archiveproductstocks where code = '" + g + "'";
                cmd = new MySqlCommand(sql, conn);
                result = cmd.ExecuteNonQuery();
                if (result > 0)
                {
                    sql = "DELETE FROM `archiveproductstocks` WHERE  code = '" + g + "'";
                    cmd = new MySqlCommand(sql, conn);
                    result = cmd.ExecuteNonQuery();
                    if (result > 0)
                    {
        ...
        void delete_click(object sender, EventArgs e)
        {
            ...
            DialogResult dial = MessageBox.Show("Are you sure you want to delete this item permanently?", "Delete", MessageBoxButtons.YesNo);
            if (dial == DialogResult.Yes)
            {
                sql = "DELETE FROM `archiveuniqueproductlist` WHERE  code = '" + g + "'";
                cmd = new MySqlCommand(sql, conn);
                ...
                {
                    MessageBox.Show("Data Deleted");
                    Inventory fr = new Inventory();
                    fr.Show();
                    fr.btnList.PerformClick();
                }
            }
        }
